...
import {funcAdditional} from "../../Request/fetchAdditional";
import './tabs.css';
    ...
    render() {
        ...
        return (
            <Fragment>
                ...
                <button className={'tabButs '+(state.whatButton.shown==='ADDITIONALLY'?'gray':'inherit')} onClick={()=>funcAdditional('ADDITIONALLY')()}>Дополнительно</button>
                <button className={'tabButs '+(state.whatButton.shown==='FOLLOWERS'?'gray':'inherit')} onClick={()=>funcAdditional('FOLLOWERS')()}>Последователи</button>
                <button className={'tabButs '+(state.whatButton.shown==='REPOS'?'gray':'inherit')} onClick={()=>funcAdditional('REPOS')()}>Репозитории</button>
