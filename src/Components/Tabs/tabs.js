...
import {funcRepos, funcFollowers, funcAddit} from "../../Request/fetchAdditional";
import './tabs.css';
    ...
    render() {
        ...
        return (
            <Fragment>
                ...
                <button className={'tabButs '+(state.whatButton.shown==='FOLLOWERS'?'gray':'inherit')} onClick={()=>funcFollowers()}>Последователи</button>
                <button className={'tabButs '+(state.whatButton.shown==='REPOS'?'gray':'inherit')} onClick={()=>funcRepos()}>Репозитории</button>
